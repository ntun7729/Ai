...
const ADMIN_KEY_STORAGE = "ai-chat.admin.provider-key.v1";
...
export function getAdminSettings() {
  ...
  return {
    ...
    providerToken: adminStatus.authenticated ? sessionStorage.getItem(ADMIN_KEY_STORAGE) || "" : "",
...
export function isAdminAuthenticated() {
  return Boolean(adminStatus.authenticated);
}
...
export function setupAdminPanel() {
  ...
  adminReadyPromise = refreshAdminStatus();
  ...
  button.addEventListener("click", async () => {
    await (adminReadyPromise || refreshAdminStatus());
    openPanel(panel);
  });
}
...
function createPanel() {
  ...
  modal.innerHTML = `
    ...
    <form class="admin-login" hidden>
      <div class="admin-section">
        <h3>Login</h3>
        <label class="admin-field">
          <span>Admin password</span>
          <input id="admin-password" type="password" placeholder="Enter admin password" autocomplete="current-password" />
          <small>Set ADMIN_PASSWORD in .dev.vars locally and as a Worker secret in production.</small>
        </label>
        <p class="admin-error" hidden></p>
        <button class="admin-login-button" type="submit">Unlock admin</button>
      </div>
    </form>
    <div class="admin-settings" hidden>
      <div class="admin-session-row"><span>Admin unlocked</span><button class="admin-logout" type="button">Log out</button></div>
      <div class="admin-section">
        <h3>Provider</h3>
        <label class="admin-field">
        ...
        <label class="admin-field">
          ...
          <input id="admin-provider-key" type="password" placeholder="Optional browser-session override" autocomplete="off" />
          <small>Stored only in this browser tab session. Production should prefer Worker secrets.</small>
        </label>
        <div class="admin-warning">Do not share admin access. Provider overrides are only sent after admin login.</div>
      </div>
      <div class="admin-section">
        <h3>Tools</h3>
  ...
  const elements = {
    backdrop,
    login: modal.querySelector(".admin-login"),
    settings: modal.querySelector(".admin-settings"),
    password: modal.querySelector("#admin-password"),
    loginError: modal.querySelector(".admin-error"),
    apiBaseUrl: modal.querySelector("#admin-api-base-url"),
    providerKey: modal.querySelector("#admin-provider-key"),
    ...
    logout: modal.querySelector(".admin-logout"),
  };
  ...
  elements.logout.addEventListener("click", async () => logoutAdmin(elements));
  elements.login.addEventListener("submit", async (event) => {
    event.preventDefault();
    await loginAdmin(elements);
  });
...
function openPanel(panel) {
  ...
  showCorrectPanel(panel);
  panel.backdrop.hidden = false;
  document.body.classList.add("admin-panel-open");
  window.setTimeout(() => {
    if (adminStatus.authenticated) panel.apiBaseUrl.focus();
    else panel.password.focus();
  }, 50);
}
...
function showCorrectPanel(panel) {
  if (!adminStatus.configured) {
    panel.login.hidden = false;
    panel.settings.hidden = true;
    panel.loginError.hidden = false;
    panel.loginError.textContent = "ADMIN_PASSWORD is not configured on the server.";
    return;
  }

  panel.login.hidden = adminStatus.authenticated;
  panel.settings.hidden = !adminStatus.authenticated;
  panel.loginError.hidden = true;
  panel.loginError.textContent = "";
}

async function loginAdmin(panel) {
  panel.loginError.hidden = true;
  ...
  if (!response.ok) {
    panel.loginError.textContent = data.error || "Admin login failed";
    panel.loginError.hidden = false;
    return;
  }
...
async function logoutAdmin(panel) {
...
async function refreshAdminStatus() {
  try {
    ...
    adminStatus = { configured: false, authenticated: false };
  }
}
...
function saveFromPanel(panel) {
  ...
  const key = panel.providerKey.value.trim();
