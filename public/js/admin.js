...
function createPanel() {
  ...
  modal.innerHTML = `
    ...
    <div class="admin-header">
      <div>
        <h2>Admin panel</h2>
        <p>Local provider and crawler settings for this browser.</p>
      </div>
    ...
    <div class="admin-section">
      ...
      <label class="admin-field">
        ...
        <small>Leave empty to use the Cloudflare Worker environment variable.</small>
      </label>
      <label class="admin-field">
        <span>API key</span>
        <input id="admin-api-key" type="password" placeholder="Use Worker secret if empty" autocomplete="off" spellcheck="false" />
        <small>Stored only in this browser's localStorage. Do not use this on shared devices.</small>
      </label>
    </div>
    ...
    <div class="admin-section">
      ...
      <label class="admin-switch">
        <input id="admin-logs-enabled" type="checkbox" />
        <span class="admin-switch-track"><span></span></span>
        <span><strong>Enable logs</strong><small>Print Worker/local debug events when requests run.</small></span>
      </label>
      <label class="admin-switch">
        <input id="admin-web-fetch-enabled" type="checkbox" />
        <span class="admin-switch-track"><span></span></span>
        <span><strong>Web crawler / fetch links</strong><small>Fetch readable text from URLs you paste into chat.</small></span>
      </label>
      <label class="admin-switch">
        <input id="admin-google-search-enabled" type="checkbox" />
        <span class="admin-switch-track"><span></span></span>
        <span><strong>Google search</strong><small>Try Google Search before other search fallbacks.</small></span>
      </label>
    </div>

    <div class="admin-warning">
      Cloudflare Worker secrets are still the safest production default. This panel is for runtime overrides from your own browser.
    </div>

    <div class="admin-actions">
      <button class="admin-reset" type="button">Reset</button>
      <button class="admin-save" type="button">Save settings</button>
    </div>
  `;
  ...
  const elements = {
    ...
    apiKey: modal.querySelector("#admin-api-key"),
  ...
  backdrop.addEventListener("click", (event) => {
    if (event.target === backdrop) closePanel(elements);
  });
  window.addEventListener("keydown", (event) => {
    if (event.key === "Escape" && !backdrop.hidden) closePanel(elements);
  });
...
function saveFromPanel(panel) {
  const next = {
    ...
    apiKey: panel.apiKey.value.trim(),
...
function fillPanel(panel, settings) {
  ...
  panel.apiKey.value = settings.apiKey || "";
...
function loadSettings() {
  try {
    ...
    return {
      ...
      ...parsed,
      webFetchEnabled: parsed.webFetchEnabled !== false,
